...
Reference:
    [1] Jiaqi Ma, Zhe Zhao, Xinyang Yi, et al. Modeling Task Relationships in Multi-task Learning with
        Multi-gate Mixture-of-Experts[C] (https://dl.acm.org/doi/10.1145/3219819.3220007)
    [2] Anonymous Authors. Noisy-Gate MMoE: Improving Multi-task Learning with Learnable Gaussian
        Noise in Mixture-of-Experts Gating. 2026.
"""
import torch
...
class MMOE_n(BaseModel):
    ...
    :param task_types: list of str, indicating the loss of each tasks, ``"binary"`` for  binary logloss,
        ``"regression"`` for regression loss. e.g. ['binary', 'regression'].
    :param task_names: list of str, indicating the predict target of each tasks.
    :param device: str, ``"cpu"`` or ``"cuda:0"``.
    :param gpus: list of int or torch.device for multiple gpus. If None, run on `device`.
        `gpus[0]` should be the same gpu with `device`.
    :param noisy_gate: bool, whether to use Noisy-Gate MMoE (NG-MMoE). If True, learnable Gaussian
        noise is injected into gate logits during training only (inference is deterministic). Default False.
    :param noise_temperature: float, global temperature tau scaling the noise standard deviation.
        Only used when noisy_gate=True. Default 1.0.
    ...
    def __init__(self, dnn_feature_columns, num_experts=3, expert_dnn_hidden_units=(256, 128),
                 ...
                 noisy_gate=False, noise_temperature=1.0):
        super(MMOE_n, self).__init__(linear_feature_columns=[], dnn_feature_columns=dnn_feature_columns,
        ...
        # NG-MMoE: one noise-scale linear layer per task: W_sigma_k in R^(num_experts x input_dim)
        # Takes the same input as the gate and predicts a per-expert noise std via softplus.
        # Paper Eq. (4): sigma_k(x) = tau * softplus(W_sigma_k * x)
        # Input dim matches the gate final layer input (after optional gate DNN).
        if self.noisy_gate:
            gate_input_dim = gate_dnn_hidden_units[-1] if len(gate_dnn_hidden_units) > 0 else self.input_dim
            self.noise_scale_layer = nn.ModuleList(
                [nn.Linear(gate_input_dim, self.num_experts, bias=False) for _ in range(self.num_tasks)])
    ...
    def forward(self, X):
        ...
        for i in range(self.num_tasks):
            if len(self.gate_dnn_hidden_units) > 0:
                ...
                gate_logits = self.gate_dnn_final_layer[i](gate_dnn_out)
            else:
                gate_dnn_out = dnn_input  # used below for noise scale input
                gate_logits = self.gate_dnn_final_layer[i](dnn_input)

            if self.noisy_gate and self.training:
                # Paper Eq. (4): sigma_k(x) = tau * softplus(W_sigma_k * gate_input)
                # gate_dnn_out holds the pre-final-layer representation (or raw dnn_input if no gate DNN)
                noise_scale = self.noise_temperature * torch.nn.functional.softplus(
                    self.noise_scale_layer[i](gate_dnn_out))  # (bs, num_experts)

                # Reparameterization trick: epsilon = sigma_k(x) * z, z ~ N(0, I)
                # This keeps gradients flowing through noise_scale_layer (W_sigma_k)
                z = torch.randn_like(noise_scale)  # (bs, num_experts)
                epsilon = noise_scale * z           # (bs, num_experts)

                # Paper Eq. (3): g~_k(x) = softmax(W_g_k * x + epsilon_k(x))
                gate_logits = gate_logits + epsilon

            # At inference (self.training=False) gate_logits is unperturbed — zero overhead
            gate_mul_expert = torch.matmul(gate_logits.softmax(1).unsqueeze(1), expert_outs)  # (bs, 1, dim)
